...
$(document).ready(function () {
  ...
  $("#patientRegistrationForm").submit((event) => {
  ...
  const isElderlyPatient = function (patient) {
    ...
    const strYearOfBirth = patient.dateOfBirth.substr(0, 4); //1980
    const nMonthOfBirth = parseInt(patient.dateOfBirth.substr(5, 2)); //04
    const nDateOfMonthBirth = parseInt(patient.dateOfBirth.substr(8, 2)); //20
    ...
    if (
      monthDiff < 0 ||
      (monthDiff === 0 && todaysDate.getDate() < patientsBirthDate.getDate())
    ) {
      age--;
    }
    return age >= 65;
